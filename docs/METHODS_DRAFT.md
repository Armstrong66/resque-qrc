...
The Hamiltonian sweep evaluates the Cartesian product of configured $J$ and $h$ values under the active encoding. A separate topology experiment compares chain and all-to-all simulator couplings. Qubit scaling assesses configured reservoir sizes on a common calibration sequence. Shot ablation compares exact expectation values with finite-shot estimates. These are calibration/robustness measurements, not substitutes for the final held-out test evaluation.
